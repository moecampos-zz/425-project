...
source('../R/stop_words.R')
```

```{r load_data}
listings <- readr::read_csv('../data/listings.csv')
reviews <- readr::read_csv('../data/reviews_with_language_textcat.csv') %>% 
...
glmnet_est <-  cv.glmnet(x = dtm, y = reviews[['price']], 
                         family = 'gaussian', 
                         alpha = 1,
...
```{r}
coefs <- coef(glmnet_est)
rownames(coefs)[order(coefs, decreasing = TRUE)][1:100]
```